...
OUT_DIR="${1:-$ROOT_DIR/build/slang_ssim}"
if [[ "$OUT_DIR" != /* ]]; then
...
SLANG_SRC="$ROOT_DIR/slang/ssim_kernels.slang"
...
if command -v slangc >/dev/null 2>&1; then
  ...
  SLANG_SRC_ARG="/work/slang/ssim_kernels.slang"
  OUT_DIR_ARG="/work/$OUT_DIR_REL"
elif [[ -f "$DOCKERFILE_PATH" ]] && command -v docker >/dev/null 2>&1; then
  echo "[slang-ssim] building docker image slang-slang from $DOCKERFILE_PATH"
...
for entry in "${entries[@]}"; do
  ...
  case "$entry" in
    ssim_forward)
      ...
      ;;
    ssim_backward)
      input_names="ssb_gradOutput_1,ssb_img1_1,ssb_img2_1,ssb_window_1,ssb_mu1_1,ssb_mu2_1,ssb_sigma1_1,ssb_sigma2_1,ssb_sigma12_1,ssb_params_1"
...
echo "[slang-ssim] done"
echo "[slang-ssim] generated JSON copied to: $BUNDLE_DIR"
