...
class Camera {
    ...
    init(
        ...
        self.imageWidth = width
        self.imageHeight = height
        self.focalX = MLXArray(focalX)
        self.focalY = MLXArray(focalY)
        self.FoVx = focal2fov(focal: self.focalX, pixels: Double(width))
        self.FoVy = focal2fov(focal: self.focalY, pixels: Double(height))
        let worldViewTransform = c2w.inverse.transpose
        let projectionMatrix = getProjectionMatrix(
            znear: znear, zfar: zfar, fovX: Double(FoVx.item(Float.self)),
            fovY: Double(FoVy.item(Float.self)))
        self.worldViewTransform = worldViewTransform.toMLXArray()
    ...
    -> matrix_double4x4
{
    ...
    P.columns = (
        ...
        SIMD4<Double>(
            (right + left) / (right - left), (top + bottom) / (top - bottom), zfar / (zfar - znear),
            1),
