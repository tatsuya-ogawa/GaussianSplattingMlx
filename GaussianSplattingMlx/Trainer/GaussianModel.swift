...
func normalizeRows(_ x: MLXArray, axis: Int = 1, eps: Float = 1e-8) -> MLXArray {
  // x: [N, ...]
  let norm = MLX.sqrt(MLX.sum(MLX.square(x), axes: [axis], keepDims: true))
  return x / (norm + eps)
}
...
class GaussModel {
  ...
  static func get_scales_from(_ scales: MLXArray) -> MLXArray {
    MLX.exp(scales)
  }
  static func get_rotation_from(_ rotation: MLXArray) -> MLXArray {
    normalizeRows(rotation)
  }
  static func get_xyz_from(_ xyz: MLXArray) -> MLXArray {
    xyz
  }
  static func get_features_from(_ features_dc: MLXArray, _ features_rest: MLXArray) -> MLXArray {
    return MLX.concatenated([features_dc, features_rest], axis: 1)
  }
  static func get_opacity_from(_ opacity: MLXArray) -> MLXArray {
    MLX.sigmoid(opacity)
  }
