...
class GaussianRenderer {
  ...
  init(active_sh_degree: Int, W: Int, H: Int, TILE_SIZE: TILE_SIZE_H_W, whiteBackground: Bool) {
  ...
  func renderTile(
    ...
    let tileSizeRest = TILE_SIZE_H_W(
      w: Swift.min(self.W - w, tileSize.w), h: Swift.min(self.H - h, tileSize.h))
  ...
  func forward(
    camera: Camera, means3d: MLXArray, shs: MLXArray, opacity: MLXArray, scales: MLXArray,
    rotations: MLXArray
  ) -> (
    render: MLXArray,
    depth: MLXArray,
