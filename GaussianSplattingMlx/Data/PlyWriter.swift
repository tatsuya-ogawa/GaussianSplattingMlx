...
class PlyWriter {
    ...
    static func writeGaussianBinary(
        positions: [[Float]],  // N×3
        features_dc: [[Float]],  // N×3 (float, f_dc_0/1/2)
        features_rest: [[Float]],  // N×M (float, f_rest_0..._M-1)
        opacities: [Float],  // N
        scales: [[Float]],  // N×3
        rotations: [[Float]],  // N×4
        to url: URL
        ...
        let numRest = features_rest.first?.count ?? 0

        guard
            features_dc.count == numPoints,
            ...
            throw NSError(
                domain: "writeGaussianBinary",
                code: -1,
                userInfo: [
                    NSLocalizedDescriptionKey: "Attribute array size mismatch"
                ]
            )
        }
    ...
    static func writeGaussianBinary(
        ...
        let n = pos.count / 3
        let M = features_rest.shape.last!
        let rest = features_rest.reshaped([-1, M]).asArray(Float.self)
        ...
        try writeGaussianBinary(
            ...
            features_rest: rest.chunked(into: M),
            opacities: opa,
    ...
    static func loadGaussianBinaryPLY(from url: URL) throws -> GaussianPlyData {
        ...
        for line in headerStr.split(separator: "\n") {
            ...
            } else if parts.count == 3 && parts[0] == "property"
                && parts[1] == "float"
            {
        ...
        func idx(_ name: String) -> Int {
            fieldNames.firstIndex(of: name)!
        }
        // [x, y, z, f_dc_0, f_dc_1, f_dc_2, ...rest..., opacity, scale_0, scale_1, scale_2, rot_0, rot_1, rot_2, rot_3]
        ...
        for i in 0..<numPoints {
            ...
            positions.append([
                floats[idx("x")], floats[idx("y")], floats[idx("z")],
            ])
            features_dc.append([
                floats[idx("f_dc_0")], floats[idx("f_dc_1")],
                floats[idx("f_dc_2")],
            ])
            var restVec = [Float]()
            for ri in 0..<restCount {
                let rName = "f_rest_\(ri)"
                restVec.append(floats[idx(rName)])
            }
            features_rest.append(restVec)
            opacities.append(floats[idx("opacity")])
            scales.append([
                floats[idx("scale_0")], floats[idx("scale_1")],
                floats[idx("scale_2")],
            ])
            rotations.append([
                floats[idx("rot_0")], floats[idx("rot_1")],
                floats[idx("rot_2")], floats[idx("rot_3")],
            ])
        }
    ...
    static func loadGaussianBinaryPLYAsMLX(from url: URL) throws -> (
        ...
        return (
            ...
            features_rest: toMLX2D(ply.features_rest),
